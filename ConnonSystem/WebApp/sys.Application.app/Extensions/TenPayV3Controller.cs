...
using sys.Application.app.Controllers;
using sys.Util.WeChatSDK;
using ZXing;
using ZXing.Common;


namespace sys.Application.app.Extensions
{
    ...
    public class TenPayV3Controller : BaseController
    {
        ...
        public static TenPayV3Info TenPayV3Info
        {
            get
            {
                ...
                {
                    _tenPayV3Info =  TenPayV3InfoCollection.Data[WeixinConfig.Instance.MchId];
                }
        ...
        {
            var returnUrl = WeixinConfig.Instance.WeixinServer + "/TenPayV3/JsApi";
            var state = $"{productId}";
            var url = OAuthApi.GetAuthorizeUrl(WeixinConfig.Instance.AppId, returnUrl, state, OAuthScope.snsapi_userinfo);
            return Redirect(url);
        }
        #region JsApi支付
        ...
        public ActionResult JsApi(string code, string state)
        {
            ...
            //var model = WeixinPayHelper.GetPayInfo("1", "aa", "xx", code);
            //return View(model);
            return View();
        }
   ...
   <return_msg><![CDATA[{1}]]></return_msg>
        ...
        //    {
        //        //在PC端打开，提供二维码扫描进行支付
        //        //return System.Web.UI.WebControls.View(product);
        //        return View(product);
        //    }
        //}
